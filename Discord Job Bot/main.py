...
async def job(
    ...
    embed.add_field(name="🕓 Expires On", value=(datetime.utcnow() + timedelta(days=duration_days)).strftime("%Y-%m-%d"), inline=False)
...
async def list_jobs(interaction: discord.Interaction):
    active_jobs = [job for job in jobs if not job.claimed and job.expires_at > datetime.utcnow()]
    if not active_jobs:
